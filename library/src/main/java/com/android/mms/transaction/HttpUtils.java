...
public class HttpUtils {
    ...
    public static byte[] httpConnection(Context context, long token,
        ...
        try {
            OkHttpClient client = createHttpClient(isProxySet, proxyHost, proxyPort);
            ...
            if (extraHttpParams != null) {
                @SuppressWarnings("deprecation") @SuppressLint({"MissingPermission", "HardwareIds"}) String line1Number = ((TelephonyManager)context
            ...
            req.addHeader(HDR_KEY_USER_AGENT, MmsConfig.getUserAgent());

            Response response = client.newCall(req.build()).execute();
            try (ResponseBody responseBody = response.body()) {
                if (!response.isSuccessful()) { // HTTP 200 is success.
    ...
    private static OkHttpClient createHttpClient(boolean isProxySet, String proxyHost, int proxyPort) {
        OkHttpClient client = new OkHttpClient();

        // set the socket timeout
        int soTimeout = MmsConfig.getHttpSocketTimeout();
        client.setConnectTimeout(soTimeout, TimeUnit.MILLISECONDS);
        client.setReadTimeout(soTimeout, TimeUnit.MILLISECONDS);
